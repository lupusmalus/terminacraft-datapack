execute in minecraft:termina run weather clear
execute in minecraft:overworld run weather clear



# pushes moon down
execute in minecraft:termina run clone -759 140 -347 -719 154 -303 -759 100 -347 replace move
execute in minecraft:termina run clone -759 155 -347 -719 169 -303 -759 115 -347 replace move
execute in minecraft:termina run clone -759 170 -347 -719 180 -303 -759 130 -347 replace move



####### CLOCKTOWER
execute in minecraft:termina run clone -741 35 -297 -739 39 -295 -741 58 -297 replace normal
execute in minecraft:termina run clone -751 28 -253 -737 42 -253 -747 62 -273 replace normal



execute as @p run ex run DAY_THIRD
execute as @p run function mm:dialogue/clear_chat